...
async def _screen_and_save(candidate_id: str, pdf_bytes: bytes, job_id: str, db_session_factory):
...
async def bulk_upload_resumes(
    ...
    for file in files:
        ...
        candidate = Candidate(
            ...
            name="Processing...", resume_url=f"uploads/{job_id}/{file.filename}",
        )
        db.add(candidate)
        application = Application(
            id=str(uuid.uuid4()), tenant_id=DEMO_TENANT,
            job_id=job_id, candidate_id=candidate.id, stage=PipelineStage.applied,
        )
        db.add(application)
        await db.flush()

        background_tasks.add_task(
            _screen_and_save, candidate_id=candidate.id,
            pdf_bytes=pdf_bytes, job_id=job_id, db_session_factory=None,
        )
        submitted.append({"file": file.filename, "status": "processing", "candidate_id": candidate.id})

    return {"submitted": len([s for s in submitted if s["status"] == "processing"]), "details": submitted}


@router.get("/{job_id}/ranked")
async def get_ranked_candidates(job_id: str, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
